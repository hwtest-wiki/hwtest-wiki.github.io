...
- [35 · 一致性测试到底是什么：为什么你的 USB、HDMI 口敢说"能和全世界的设备互通"？](/articles/35) 🔧
- [37 · 同样一个 USB 口，插 U 盘飞快、插移动硬盘却掉速——USB 2.0 的 480M 到底靠什么保证？一张眼图模板，判它"过没过"](/articles/37) 🔧
- [q02 · USB、HDMI、DP、MIPI、PCIe……这些接口到底是什么、怎么分、谁看眼图谁看协议？——模块六「串行高速接口全景地图」](/articles/q02) 🔧
- [50 · 串口收回来全是乱码、9600 能通 115200 就乱、TX/RX 接反啥都收不到——UART：连时钟线都省了的异步串口，测的是波特率误差、帧格式、采样点和协议解码，判据在两端约定不在总线规范也不在眼图](/articles/50) 🔧
- [49 · SPI 读出来全是乱码、错开一位、慢速跑得好一提速就错、换个模式又对了——SPI 总线：全双工、推挽、片选、更快，测的是边沿信号完整性、setup/hold 时序、CPOL/CPHA 模式和协议解码，判据看器件手册不是统一规范表](/articles/49) 🔧
